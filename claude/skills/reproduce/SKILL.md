---
name: reproduce
description: Independently re-derive a claimed result (proof, derivation, root-cause analysis, benchmark, "this PR fixes X") from its problem statement with the author's path and conclusion quarantined, then compare. Complements /verify — verify checks facts against sources; reproduce checks whether the derivation itself is sound. Triggers: "reproduce", "追試", "再導出", "検算", "この証明は正しい?", "この分析を独立に確かめて", or before trusting a load-bearing derivation nobody has independently checked.
---
...
Facts inside the derivation (API limits, constants, citations) route to /verify; this skill judges the logic between them.
