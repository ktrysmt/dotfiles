...
- **Agent Teams over subagents for parallel work**: When 2+ parallel tasks arise, spawn one named teammate per task with the `Agent` tool (set `name`; `subagent_type` optional), then collect their work as FILES (see the deliverable sub-bullet below). Do not substitute with multiple unnamed `Agent` calls (including `run_in_background`). Reason: a teammate is a full session that loads CLAUDE.md, MCP servers, and skills on its own, and teammates can message each other directly, whereas a subagent only reports back to the caller. Single `Agent` calls are exempt from this restriction.
